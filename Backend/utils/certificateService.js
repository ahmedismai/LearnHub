...
const PLAYWRITE_FONT_URL = "https://github.com/google/fonts/raw/main/ofl/playwriteza/PlaywriteZA-VariableFont_wght.ttf";
const OUTFIT_FONT_URL = "https://github.com/google/fonts/raw/main/ofl/outfit/static/Outfit-Regular.ttf";
const OUTFIT_BOLD_URL = "https://github.com/google/fonts/raw/main/ofl/outfit/static/Outfit-Bold.ttf";
...
export const generateAndUploadCertificate = async (studentId, courseId) => {
  try {
    ...
    try {
      ...
      console.warn("[CERT-GEN] Font loading failed, using fallbacks:", error.message);
    }
    ...
    page.drawRectangle({ x: 30, y: 30, width: width - 60, height: height - 60, borderColor: primaryBlue, borderWidth: 3 });
    page.drawRectangle({ x: 45, y: 45, width: width - 90, height: height - 90, borderColor: goldColor, borderWidth: 1.5 });

    // --- Content ---
    const brandText = "LEARNHUB ACADEMY";
    const brandWidth = boldFont.widthOfTextAtSize(brandText, 24);
    page.drawText(brandText, { x: width / 2 - brandWidth / 2, y: height - 100, size: 24, font: boldFont, color: accentTeal });

    const titleText = "CERTIFICATE OF COMPLETION";
    const titleWidth = boldFont.widthOfTextAtSize(titleText, 36);
    page.drawText(titleText, { x: width / 2 - titleWidth / 2, y: height - 160, size: 36, font: boldFont, color: primaryBlue });

    const subText = "This is to officially certify that";
    const subWidth = regularFont.widthOfTextAtSize(subText, 18);
    page.drawText(subText, { x: width / 2 - subWidth / 2, y: height - 210, size: 18, font: regularFont, color: rgb(0.4, 0.4, 0.4) });

    const nameText = student.name?.toUpperCase() || "LEARNER NAME";
    const nameWidth = boldFont.widthOfTextAtSize(nameText, 48);
    page.drawText(nameText, { x: width / 2 - nameWidth / 2, y: height - 280, size: 48, font: boldFont, color: accentTeal });
    ...
    page.drawText(completionText, { x: width / 2 - completionWidth / 2, y: height - 330, size: 16, font: regularFont, color: rgb(0.3, 0.3, 0.3) });

    const courseText = course.title || "COURSE TITLE";
    const courseWidth = boldFont.widthOfTextAtSize(courseText, 28);
    page.drawText(courseText, { x: width / 2 - courseWidth / 2, y: height - 380, size: 28, font: boldFont, color: primaryBlue });

    // --- Footer ---
    const dateText = `Date of Issue: ${new Date().toLocaleDateString()}`;
    page.drawText(dateText, { x: 100, y: 120, size: 14, font: boldFont, color: primaryBlue });
    page.drawLine({ start: { x: 100, y: 115 }, end: { x: 280, y: 115 }, thickness: 1, color: primaryBlue });
    ...
    page.drawText(`Instructor, LearnHub Academy`, { x: width - 280, y: 100, size: 12, font: regularFont, color: rgb(0.4, 0.4, 0.4) });
    page.drawLine({ start: { x: width - 280, y: 120 }, end: { x: width - 100, y: 120 }, thickness: 1, color: primaryBlue });
