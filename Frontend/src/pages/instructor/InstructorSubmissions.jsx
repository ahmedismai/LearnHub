  ...
  return (
    <div className="space-y-8 animate-fade-in max-w-7xl mx-auto pb-20">
      <div>
        ...
        <p className="text-muted-foreground">Monitor performance in AI-generated and official exams.</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        <Card className="border-none shadow-xl overflow-hidden bg-card/50 backdrop-blur-sm">
          <CardHeader className="bg-primary/5 border-b">
            ...
            <CardDescription>Track who passed and who is eligible for graduation.</CardDescription>
          </CardHeader>
          <CardContent className="p-0">
            <Table>
              <TableHeader>
                <TableTableRow className="bg-muted/30">
              ...
              <TableBody>
                {submissions.length > 0 ? (
                  submissions.map((sub) => (
                    <TableTableRow key={sub._id} className="hover:bg-muted/10 transition-colors">
                      <TableCell>
                        <div className="flex items-center gap-3">
                           ...
                           <div>
                             <p className="font-bold text-sm">{sub.studentId?.name}</p>
                             <p className="text-[10px] text-muted-foreground">{sub.studentId?.email}</p>
                           </div>
                        </div>
                      </TableCell>
                      <TableCell>
                         <p className="font-semibold text-sm">{sub.courseId?.title}</p>
                         <p className="text-xs text-primary">{sub.examId?.title}</p>
                      </TableCell>
                      <TableCell>
                         <div className="flex flex-col">
                            <span className={`text-lg font-black ${sub.score >= 70 ? 'text-green-500' : 'text-destructive'}`}>
                               {sub.score}%
                            </span>
